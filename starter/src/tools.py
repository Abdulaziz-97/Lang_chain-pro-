...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        This tool performs mathematical calculations on expressions containing
        basic arithmetic operations. Only safe characters are allowed for security.
        
        Args:
            expression: The mathematical expression to evaluate. 
                       Supports: digits (0-9), operators (+, -, *, /), 
                       parentheses, decimal points, and spaces.
                       Examples: "1 + 1", "2 * 3.5", "(10 + 5) / 3"
        
        Returns:
            A formatted string with the calculation result or an error message
        """
        try:
            # Validate expression for security - only allow safe characters
            import re
            if not re.match(r'^[\d\s+\-*/().]+$', expression):
                error_msg = "Invalid expression: only digits, +, -, *, /, parentheses, and spaces are allowed"
                logger.log_tool_use(
                    "calculator",
