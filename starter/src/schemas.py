...
class AnswerResponse(BaseModel):
    ...
    confidence: float = Field(ge=0.0, le=1.0, description="The confidence in the answer (between 0 and 1)")
...
class UserIntent(BaseModel):
    ...
    intent_type: Literal["qa", "summarization", "calculation", "unknown"] = Field(
        description="The type of intent (qa, summarization, calculation, or unknown)"
    )
    confidence: float = Field(ge=0.0, le=1.0, description="The confidence in the intent (between 0 and 1)")
    reasoning: str = Field(description="The reasoning for the intent")
